...
#include <signal.h>
#include <map>
#include <cerrno>   // errno, ETIMEDOUT
...
public:
    ...
    // 暂停或恢复线程：线程未启动时返回错误；运行时发送SIGUSR1触发暂停/恢复
    int Pause() {
        if (m_thread == 0) return -1; // 线程尚未启动
        if (m_bpaused) {
            m_bpaused = false;
            return 0;
        }
...
private:
    ...
    static void* ThreadEntry(void* arg) {
        ...
        pthread_t thread = pthread_self();// Stop函数可能已将 m_thread 清空
    ...
    // 信号处理：SIGUSR1用于暂停/恢复，SIGUSR2用于请求退出
